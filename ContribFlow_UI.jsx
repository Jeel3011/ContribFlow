...
const Spinner = () => (
  <div style={{ display: "flex", alignItems: "center", gap: 12, color: T.blue,
    ...
    <div style={{
      ...
      border: `3px solid ${T.blueGlow}`, borderTopColor: T.blue,
      animation: "spin 0.8s linear infinite"
    }} />
    <span style={{ background: "linear-gradient(90deg, #2563EB, #0891B2)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
      Agent is reasoning…
    </span>
  </div>
);
...
function StageCard({ stageName, status, summary, data, defaultOpen = false }) {
        ...
        <div style={{ padding: "16px 20px", borderTop: `1px solid ${T.border}` }} className="slide-down">
          ...
          {stageName === "Idea Deduplication" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <div style={{ fontSize: 13, color: T.text, padding: 12, background: data.status === "conflict" ? "#FEF2F2" : "#F0FDF4", borderRadius: 8, border: `1px solid ${data.status === "conflict" ? "#FCA5A5" : "#86EFAC"}` }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                  {(data.services_at_risk || []).length === 0 ? <span style={{fontSize: 12, color: T.dim}}>None identified</span> : 
                    data.services_at_risk.map((s, i) => (
                      <span key={i} style={{ fontSize: 11, fontWeight: 600, padding: "5px 12px", background: "linear-gradient(135deg, #FEF3C7, #FDE68A)", color: "#92400E", borderRadius: 20, border: "1px solid #F59E0B" }}>{s}</span>
                    ))
                  }
                </div>
              </div>
          ...
          {stageName === "Pre-PR Quality Check" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              {(data.issues || []).length === 0 && (
                <div style={{ fontSize: 13, color: T.green, padding: 12, background: "#F0FDF4", borderRadius: 8, border: `1px solid #86EFAC` }}>
    ...
    <div className="fade-in">
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 24 }}>
        {Object.entries(stages)
          ...
          .map(([key, s]) => (
            <StageCard key={key} stageName={s.stageName} status={s.status}
